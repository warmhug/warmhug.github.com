...
而近几个月同样发生着另外一个有趣又类似的大事：在后端被广泛使用的 Rest (RESTful API)，开始受到各种质疑，特别是声称解决或替代 Rest 的方案如 [GraphQL](http://facebook.github.io/graphql/)、[Falcor](http://netflix.github.io/falcor/) 的出现并得到越来越多的关注，似乎问题来了：Rest 是否也像 Angular.js 那样，要死了？
...
另外这里引用 Falcor 作者提到的 Rest 的设计初衷（[视频](https://youtu.be/hOE6nVVr14c?t=5m48s)）：
...
但如果对 Rest 的实现并不充分的话，也会不能明显感受到其好处。这里引用下 [richardson模型](http://martinfowler.com/articles/richardsonMaturityModel.html)，其中介绍了怎么更充分的实现 Rest：
